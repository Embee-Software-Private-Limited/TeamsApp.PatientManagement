...
namespace Bansal.TeamsApp.PatientManagement.Bot.Authentication
{
    ...
    public class MSGraphScopeHandler : AuthorizationHandler<MSGraphScopeRequirement>
    {
        ...
        {
            var accessToken = await this.tokenAcquisition.GetAccessTokenForUserAsync(new[] { Bot.Constants.ScopeGroupReadAll });
            var tokenHandler = new JwtSecurityTokenHandler();
            var securityToken = tokenHandler.ReadToken(accessToken) as JwtSecurityToken;
            var claimValue = securityToken.Claims
                .First(claim => claim.Type.Equals(Bot.Constants.ClaimTypeScp.ToString(), StringComparison.CurrentCultureIgnoreCase)).Value;
